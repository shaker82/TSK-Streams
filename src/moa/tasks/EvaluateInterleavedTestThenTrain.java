...
public class EvaluateInterleavedTestThenTrain extends MainTask {
    ...
    public FileOption predictionFileOption = new FileOption("predictionFile", 'p',
            "File to output the produced predictions", null, "txt", true);
...
//        ExampleStream stream = (InstanceStream) getPreparedClassOption(this.streamOption);
        ...
        File predictionFile = this.predictionFileOption.getFile();
        PrintStream predictionResultStream = null;

        if (predictionFile != null) {
            try {
                if (predictionFile.exists()) {
                	predictionResultStream = new PrintStream(
                            new FileOutputStream(predictionFile, true), true);
                } else {
                	predictionResultStream = new PrintStream(
                            new FileOutputStream(predictionFile), true);
                }
            } catch (Exception ex) {
                throw new RuntimeException(
                        "Unable to open immediate result file: " + predictionFile, ex);
            }
        }
        ...
        while (stream.hasMoreInstances()
            ...
            ((Instance)testInst.getData()).setMissing(((Instance)testInst.getData()).classIndex());
                  	
            double[] prediction = learner.getVotesForInstance(testInst);
            ((Instance)testInst.getData()).setClassValue(trueClass);

            if (Double.isNaN(prediction[0]))
            	learner.getVotesForInstance(testInst);

            if (Math.abs(prediction[0]-1428564)<2)
            	learner.getVotesForInstance(testInst);
...
//no need for this string to be written in the file
//                    immediateResultStream.print(learnerString + "," + streamString + "," + this.randomSeedOption.getValueAsCLIString() + ",");
            ...
            if (predictionResultStream != null) {
            	predictionResultStream.println(prediction[0]);
            	predictionResultStream.flush();
            }
        }
        if (immediateResultStream != null) {
